...
int	calc_turns(t_path *data, int *paths)
{
	...
	{
		if (data->adj_grid[data->end][i] && paths[i]) //KAY LAPI
		{
			path_len = pathlen(paths, i, data->start); //KAY LAPI
...
void	get_winner(t_path *data, int *winner, int *curr_path)
{
	...
	printf("-------------------\n");
	printf("\nturns: %i\n\n", turns);
	if (turns < data->best_turn_count) //KAY LAPI
	{
		ft_memmove(winner, curr_path, sizeof(int) * data->room_count);
		data->best_turn_count = turns; //KAY LAPI
	}
}
